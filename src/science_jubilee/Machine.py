...
class Machine:
    ...
    def load_labware(
        self, labware_filename: str, slot: int, has_lid_on_top: bool = False, currentLiquidVolume: float= 0, path: str = None, order: str = "rows"
    ):
        ...
        if path is not None:
            labware = self.deck.load_labware(
                labware_filename, slot, has_lid_on_top, currentLiquidVolume, path=path, order=order
            )
        else:
            labware = self.deck.load_labware(labware_filename, slot, has_lid_on_top, currentLiquidVolume, order=order)
